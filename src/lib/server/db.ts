...
// The database will now be opened and closed on each transaction.
...
async function _prune_old_performance_data(db: Database) {
...
export async function storePerformanceMetrics(server_id: string, timestamp: string, data: DashboardData) {
    ...
    try {
        ...
        if (perf_data) {
            ...
            for (const stats of io_details) {
                 await db.run(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)`,
                    [server_id, timestamp, stats.device, stats.mount_point, stats.read_mb_s, stats.write_mb_s]
                );
            }
        }
        ...
        await _prune_old_performance_data(db);
    } finally {
        await db.close();
    }
}
...
export async function getPerformanceHistory24h(server_id: string): Promise<PerformanceHistory> {
    ...
    try {
        ...
        for (const row of wait_event_rows) {
            ...
            events_by_name[event_name].data!.push({ date: row.timestamp, value: row.session_count, latency: row.latency_seconds });
        }
        ...
        for (const row of io_detail_rows) {
            ...
            io_details_map[row.timestamp].push({ 'device': row.device, 'mount_point': row.mount_point, 'read_mb_s': row.read_mb_s, 'write_mb_s': row.write_mb_s });
        }
        ...
        for (const row of summary_rows) {
            ...
            performance_data.io_read!.push({ date: ts, value: row.io_read_total, details: io_details_map[ts] || [] });
            performance_data.io_write!.push({ date: ts, value: row.io_write_total, details: io_details_map[ts] || [] });
        }
